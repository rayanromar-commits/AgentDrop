...
def _put_file(upload_url: str, file_path: Path) -> None:
    ...
    if resp.status_code not in (200, 201, 206):
        raise RuntimeError(
            f"TikTok file upload failed ({resp.status_code}): {resp.text[:300]}"
        )
...
def upload_video_tiktok(video_row, config: dict) -> str:
    ...
    size = file_path.stat().st_size
    # Single-chunk upload (our videos are well under TikTok's 64MB limit).
    source_info = {
        "source": "FILE_UPLOAD",
        "video_size": size,
        "chunk_size": size,
    ...
    _put_file(data["upload_url"], file_path)
